const muQuizApp = document.querySelector('.muQuizApp');
...
myBtn.onclick = () => {
    ...
    muQuizApp.style.display = 'none';
}

exitBtn.onclick = () => {
    myRulesBox.classList.remove('activeInfo');
    muQuizApp.style.display = 'block';
}
...
const optionSelected = (option) => {
    ...
    if (userAns === correctAns) {
        ...
        for (let index = 0; index < allOptions; index++) {
            if (myOptions.children[index].innerText === correctAns) {
                myOptions.children[index].classList.add('correct');
                myOptions.children[index].innerHTML += checkIcon;
            }
        }
    }

    for (let index = 0; index < allOptions; index++) {
        myOptions.children[index].classList.add('disabled');
    }

    nextBtn.style.display = 'block';
}

const autoSelectAnswer = () => {
    if (counterOne === questionLen) {
